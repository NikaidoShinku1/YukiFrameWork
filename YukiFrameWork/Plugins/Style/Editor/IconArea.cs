...
{
    public class IconArea : IconWindowArea
    {
        private int toolbarIndex;
        private SearchField searchField;
        private string searchString;

        private List<GUIContent> smallIconList;
        private List<GUIContent> bigIconList;

        private int smallCount;
        private int bigCount;
        private Vector2 scrollPos;
        private float smallSize = 40;
        private float bigSize = 70;
        private float smallPreviewHeight = 120;
        private float bigPreviewHeight = 140;
        private float curPreviewHeight;
        private float curSize;
        private int xCount;
        private const int iconSpace = 5;
        private int selectedIconIndex;
        private GUIContent selectedIcon;
        ...
        protected override void OnInit()
        {
            ...
            smallIconList = new List<GUIContent>();
            bigIconList = new List<GUIContent>();
            GUIContent iconContent;
            foreach (var name in iconNameList)
            {
                Debug.unityLogger.logEnabled = false;
                iconContent = EditorGUIUtility.IconContent(name);
                iconContent.tooltip = name;
                Debug.unityLogger.logEnabled = true;
                if (iconContent == null || iconContent.image == null) continue;
                if (iconContent.image.width <= 36 || iconContent.image.height <= 36) smallIconList.Add(iconContent);
                else bigIconList.Add(iconContent);
            }
            smallCount = smallIconList == null ? 0 : smallIconList.Count;
            bigCount = bigIconList == null ? 0 : bigIconList.Count;
        }
        ...
        void OnSmallContentGUI()
        {
            if (smallCount == 0) return;
            curSize = smallSize;
            curPreviewHeight = smallPreviewHeight;
            mainRect = new Rect(areaRect.x, areaRect.y, areaRect.width, areaRect.height - curPreviewHeight);
            OnIconGUI(smallIconList);
        }

        void OnBigContentGUI()
        {
            if (bigCount == 0) return;
            curSize = bigSize;
            curPreviewHeight = bigPreviewHeight;
            mainRect = new Rect(areaRect.x, areaRect.y, areaRect.width, areaRect.height - curPreviewHeight);
            OnIconGUI(bigIconList);
        }

        void OnIconGUI(List<GUIContent> iconList)
        {
            int index = 0;
            xCount = Mathf.CeilToInt(areaSize.x / (curSize + iconSpace));
            if (xCount <= 0) return;
            while (index < iconList.Count)
            {
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    {
                        if (DoesContentMatchSearch(iconList[index], searchString))
                        {
                            if (index == selectedIconIndex) GUI.backgroundColor = selectedColor;
                            if (GUILayout.Button(iconList[index], GUILayout.Width(curSize), GUILayout.Height(curSize)))
                            {
        ...
        void OnSelectIconGUI()
        {
            if (selectedIcon == null) return;
            previewRect = new Rect(areaRect.x, areaRect.y + mainRect.height, areaRect.width, areaRect.height - mainRect.height);
            GUILayout.BeginArea(previewRect);

            EditorGUILayout.BeginHorizontal();

            EditorGUILayout.BeginVertical(GUILayout.Width(70));
            GUILayout.Button(selectedIcon, GUILayout.Width(70), GUILayout.Height(70));
            if (GUILayout.Button("Copy", GUILayout.Width(70))) MEditorUtility.CopyString(selectedIcon.tooltip);
            EditorGUILayout.EndVertical();

            EditorGUILayout.BeginVertical();
            OnIconItemInfoGUI("Name", selectedIcon.tooltip);
            OnIconItemInfoGUI("Load API", $"EditorGUIUtility.IconContent(\"{selectedIcon.tooltip}\");");
            EditorGUILayout.EndVertical();
        ...
        void OnIconItemInfoGUI(string key, string value)
        {
            EditorGUILayout.BeginHorizontal();
        ...
        bool DoesContentMatchSearch(GUIContent content, string search)
        {
            if (string.IsNullOrEmpty(search)) return true;
            return content.tooltip.IndexOf(search, StringComparison.OrdinalIgnoreCase) >= 0;
        }
