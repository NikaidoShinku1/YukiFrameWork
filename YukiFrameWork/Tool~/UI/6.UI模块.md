...
|bool IsPanelExist<T>() where T : BasePanel|全层级查找场景中是否有面板实例，如果有，即使是缓存关闭状态也返回True|
|bool IsPanelExist<T>(UILevel level) where T : BasePanel|全层级查找场景中是否有面板实例，如果有，即使是缓存关闭状态也返回True|
|UILoadAssetRequest PreLoadPanelAsync<T>(string name = "",params object[] param) where T : BasePanel|异步预加载面板，面板默认是关闭的|
|T PreLoadPanel<T>(string name = "", params object[] param) where T : BasePanel|预加载面板，面板默认是关闭的|
...
附带模块：为对象添加AspectAdapted组件可自动进行UI分辨率适配

通过左上角YukiFrameWork/UI检索窗口打开面板可检查所有的UI
