...
UIKit static API:
    ...
    - T OpenPanel<T>(string name) where T : BasePanel//根据名称/路径打开面板

    - T OpenPanel<T>(string name,UILevel level) where T : BasePanel//可动态修改层级的重载

    - T OpenPanel<T>(T panel) where T : BasePanel//直接传递面板的预制体进行克隆创建面板

    - T OpenPanel<T>(T panel,UILevel level) where T : BasePanel//可动态修改层级的重载

    - void OpenPanelAsync<T>(string name,Action<T> onCompleted) where T : BasePanel//异步打开面板，需要传入回调

    - UILoadAssetRequest OpenPanelAsync<T>(string name) where T : BasePanel//有返回值的异步打开面板
