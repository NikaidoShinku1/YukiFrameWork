...
|void StopAllMusic()|关闭所有的Music层播放|
|void StopAllVoice()|关闭所有的Voice层播放|
|void StopAllSound()|关闭所有的Sounc层播放|
...
|BindablePropertyPlayerPrefsByFloat AudioVolumeScale { get;}|AudioKit全层级共享音量缩放值，最终播放的音效会在设定好不同分组中内定的音量后根据缩放值计算|
|BindablePropertyPlayerPrefsByFloat MusicVolumeScale { get;}|Music分组的音量缩放值,默认是1|
|BindablePropertyPlayerPrefsByFloat VoiceVolumeScale { get;}|Voice分组的音量缩放值,默认是1|
|BindablePropertyPlayerPrefsByFloat SoundVolumeScale { get;}|Sound分组的音量缩放值,默认是1|
...
|BindablePropertyPlayerPrefsByFloat AudioGroupVolumeScale { get; }|这个分组所关联的音频缩放值|
