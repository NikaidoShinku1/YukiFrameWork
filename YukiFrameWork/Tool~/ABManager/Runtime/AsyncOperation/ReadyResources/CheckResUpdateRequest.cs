...
namespace YukiFrameWork.ABManager
{
    ...
    public class CheckResUpdateRequest : CustomAsyncOperation<CheckResUpdateRequest>
    {
        ...
        internal IEnumerator CheckResUpdate(string projectName)
        {
            ...
            { 
                // 判断服务端是否有.zip文件  
                string zipPath = ABTools.ServerPath(AssetBundleManager.GetProfile(projectName).url, projectName, requestVersion.version, string.Format("{0}.zip", projectName));
            ...
            GetFileFromServerRequest requestFile = AssetBundleManager.GetFileFromServer(projectName, requestVersion.version, ABConst.project_build_info);
        ...
        private IEnumerator CompareWithLocal(string projectName, BundleInfo[] serverBundleInfos)
        {
            ...
            for (int i = 0; i < serverBundleInfos.Length; i++)
            {
                ...
                // 判断本地是否存在
                string localFile = ABTools.LocalResPath(projectName, serverBundleInfos[i].bundleName);
                string buildInPath = ABTools.BuildInDataPath(projectName, serverBundleInfos[i].bundleName);
                ...
                if (File.Exists(localFile) || File.Exists(buildInPath))
                {
                    ...
                    {
                        // 计算md5
                        FileMD5Request request_md5 = ABTools.CaculateFileMD5(localFile);
            ...
            else
            {
                ...
                if (isHaveBuiltIn.isHave && !ABTools.StreamingAssetsWritable())
                {
...
#if ABManager_LOG_OPEN_TESTING
            Debug.LogFormat("CheckResUpdate CheckLocalRes isUpdate:{0} isHaveBuiltIn:{1} IsHaveResOnLocal:{2} StreamingAssetsReadable:{3}", isUpdate, isHaveBuiltIn.isHave, AssetBundleManager.IsHaveResOnLocal(projectName), ABTools.StreamingAssetsReadable());
#endif
