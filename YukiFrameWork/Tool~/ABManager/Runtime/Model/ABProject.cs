...
namespace YukiFrameWork.ABManager
{
    ...
    /// </summary>
    [System.Serializable]
    public class ABProject : ScriptableObject
    {
        ...
        /// </summary>
        [HideInInspector]
        public List<ABAssetBundle> assetBundles;

        private Dictionary<string, ABAssetBundle> _bundles = null;

        private Dictionary<string, ABAssetBundle> Bundles
        {
            get {
                if (_bundles == null) { 
                    _bundles = new Dictionary<string, ABAssetBundle>();
        ...
        public Dictionary<string, string> AssetBundleNameMapping
        {
            get {
                if (asset_bundle_name_mapping == null || asset_bundle_name_mapping.Count == 0) 
                {
                    ...
                    List<ABAssetBundle> allAssetBundle = GetAllAssetBundles();
        ...
        public ABProject()
        {
            assetBundles = new List<ABAssetBundle>();
        ...
        public bool IsDependenceProject(string name  ) {
            ...
            {
                ABProject project = ABProjectManager.Instance.GetProject(dependenceProject[i]);
        ...
        public bool AddAssetBundle(ABAssetBundle bundle ) {
        ...
        {
            ABAssetBundle assetBundle = GetAssetBundle(originName);
            ...
            if (assetBundle != null)
            {
                ...
                _bundles = null; // 清空assetbundle的缓存
                if (assetBundle.bundleType == BundleType.Group)
                {
                    ABAssetBundle[] bundles = GetAssetBundlesFromGroup(originName);
                    if (bundles != null)
                    {
        ...
        public ABAssetBundle GetAssetBundle(string bundle_name) {
        ...
        public ABAssetBundle GetAssetBundle(int nameHashCode) {
        ...
        // 获取某个组的所有AssetBundle
        public ABAssetBundle[] GetAssetBundlesFromGroup(string group_name) {

            if (string.IsNullOrEmpty(group_name)) return null;

            List<ABAssetBundle> bundles = new List<ABAssetBundle>();
            for (int i = 0; i < assetBundles.Count; i++)
            {
        ...
        public void RemoveAssetBundle(ABAssetBundle bundle) {
        ...
        public void RemoveAssetBundle(int nameHashCode)
        {
            ...
            ABAssetBundle bundle = GetAssetBundle(nameHashCode);
            if (bundle != null)
            {
            ...
            {
                ABAssetBundle[] bundles = GetAssetBundlesFromGroup(bundle.bundle_name);
                if (bundles != null)
                { 
                    foreach (ABAssetBundle item in bundles)
                    {
        ...
        // 获取Bundle 通过 它包含的文件
        public ABAssetBundle GetBundleByContainFile(string asset_path) {
            for (int i = 0; i < assetBundles.Count; i++)
            {
        ...
        // 获取依赖的AssetBundle
        public List<ABAssetBundle> GetDependenciesBundles(ABAssetBundle bundle) {
        ...
        /// </summary>
        /// <returns></returns>
        public List<ABAssetBundle> GetAllAssetBundles() {
            ...
            foreach (var item in assetBundles)
            {
                ...
                {
                    case BundlePackgeType.One:
                        bundles.Add(item);
                        break;
                    case BundlePackgeType.Multiple:
