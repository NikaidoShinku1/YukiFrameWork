...
namespace YukiFrameWork.ABManager
{
    ...
    public class ABManagerSettings : ScriptableObject
    {
        [HideInInspector]
        [SerializeField]
        ...
        private static ABManagerSettings _instance;

        public static ABManagerSettings Instances {
            get {
                if (_instance == null) 
                {
                    // 通过Resource 加载
                    _instance = Resources.Load<ABManagerSettings>(typeof(ABManagerSettings).Name);

                    if (_instance == null) {

                        // 判断是不是编辑器模式
#if UNITY_EDITOR
                        _instance = ScriptableObject.CreateInstance<ABManagerSettings>();
                        ...
                        string path = string.Format("Assets/Resources/{0}.asset",  typeof(ABManagerSettings).Name); 
...
#else
                        throw new Exception("配置缺失:ABManagerSettings!");
#endif 
                    } 
                }
                return _instance;
