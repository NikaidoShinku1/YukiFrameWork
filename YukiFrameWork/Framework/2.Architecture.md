Architecture：框架核心架构,借鉴Qframework的层级思路而设置 架构层级为：IController,ISystem,IUtility,IModel,IConfig
...
|T GetConfig() where T : UnityEngine.Object|会自动进行类型转换的配表数据获取|
|UnityEngine.Object GetConfig()|配表数据获取|
|strign GetConfigByFile()|获取文件内数据，例如json，xml|
|---|---|
|Constructor Method API|构造函数API声明|
|public ArchitectureTable(Dictionary<string, Type> table, ArchitecTableLoadType tableLoadType)|需要传入配置的字典，以及加载方式,配置的Key为路径/名称(根据加载方式),Value为资源的类型|
|public ArchitectureTable(Dictionary<string, Type> table, Func<string, Type, UnityEngine.Object> ResLoader|需要传入配置的字典，以及自定义的加载回调|
...
``` csharp
    ...
    public class World : Architecture<World>
    {
        ...
        {
            return new ArchitectureTable(new System.Collections.Generic.Dictionary<string, Type>()
            {
                ["CustomConfig"] = typeof(TextAsset)
            },tableLoadType:ArchitecTableLoadType.XFABManager);
        }
        public override void OnInit()
        {
