...
namespace XFABManager
{
    ...
    [System.Serializable]
    public enum XFBundlePackgeType {
        /// <summary>
        /// 所有的资源打包到一个ab包中
        /// </summary>
        One,
    ...
    public class FileInfo {
        ...
        public FileInfo(string guid) {
            ...
            this.type = AssetDatabase.IsValidFolder(AssetPath) ? XFBundleFileType.Directory : XFBundleFileType.File;
        }
    ...
    public class XFABAssetBundle
    {
        ...
        public XFBundleType bundleType = XFBundleType.Bundle;
        ...
        public XFBundlePackgeType bundlePackgeType = XFBundlePackgeType.One;
        ...
        private void AddFile(FileInfo fileInfo) {
            ...
            if (fileInfo.type == XFBundleFileType.Directory)
            {
            ...
            else if ( fileInfo.type == XFBundleFileType.File ) {
        ...
        public bool IsContainFile(string guid) {
            ...
            for (int i = 0; i < files.Count; i++)
            {
                ...
                switch (files[i].type)
                {
                    case XFBundleFileType.File:
                        if (guid.Equals(files[i].guid))
                            return true;
                        break;
                    case XFBundleFileType.Directory:
        ...
        public bool IsDuplicateNames(FileInfo fileInfo) {
            ...
            for (int i = 0; i < files.Count; i++)
            {
                if (fileInfo.displayName.ToLower().Equals(files[i].displayName.ToLower()) 
                    && fileInfo.type == XFBundleFileType.File 
                    && files[i].type == XFBundleFileType.File) {
        ...
        public string GetAssetPathByFileName(string fileName,Type type)
        {
            ...
            for (int i = 0; i < files.Count; i++)
            {
                if (files[i].type == XFBundleFileType.File)
                {
                ...
                else if (files[i].type == XFBundleFileType.Directory)
                {
