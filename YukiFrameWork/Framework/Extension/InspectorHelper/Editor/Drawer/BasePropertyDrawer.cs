    ...
    public class BasePropertyDrawer : PropertyDrawer
	{
        ...
        {   
            ...
            else
            {
                ...
                EditorGUI.BeginDisabledGroup(OnDisableGroup(info));        
                if (info.Label != null)
        ...
        public virtual void InitInfo(SerializedProperty property)
        {
            ...
            if (CheckPropertyInGeneric(fieldInfo.FieldType) && listDrawerSetting != null)
            {
                var listInfo = new PropertyReorderableListDrawedInfo
                    ...
                    , property, color, enableEnumValue, disableEnumValue, enableIf, disableIf, helperBox, runtimeDisableGroup, editorDisabledGroup);
            ...
            {
                var bInfo = new PropertyBoolanDrawedInfo(property.serializedObject.targetObject, property, boolanPopup, arrayLabel, listDrawerSetting, displayTexture, propertyRange, label, color, enableEnumValue, disableEnumValue, enableIf, disableIf, helperBox, runtimeDisableGroup, editorDisabledGroup);
                bInfo.ItemType = fieldInfo.FieldType;
                this.info = bInfo;
            }
            else if (CheckPropertyInEnum(fieldInfo.FieldType, property))
            {
                var eInfo = new PropertyEnumDrawedInfo(property.serializedObject.targetObject, boolanPopup, arrayLabel, listDrawerSetting, displayTexture, label, propertyRange, property, color, enableEnumValue, disableEnumValue, enableIf, disableIf, helperBox, runtimeDisableGroup, editorDisabledGroup);
                eInfo.ItemType = fieldInfo.FieldType;
                this.info = eInfo;
            }
            else if (CheckPropertyInTexture(fieldInfo.FieldType, displayTexture))
            {
                var tInfo = new PropertyTextureDrawedInfo(property.serializedObject.targetObject, boolanPopup, arrayLabel, listDrawerSetting, displayTexture, propertyRange, property, label, color, enableEnumValue, disableEnumValue, enableIf, disableIf, helperBox, runtimeDisableGroup, editorDisabledGroup);
                tInfo.ItemType = fieldInfo.FieldType;
                this.info = tInfo;
            }
            else
            {
                var info = new PropertyMemberDrawedInfo(property.serializedObject.targetObject, boolanPopup, arrayLabel, listDrawerSetting, displayTexture, label, propertyRange, property, color, enableEnumValue, disableEnumValue, enableIf, disableIf, helperBox, runtimeDisableGroup, editorDisabledGroup);
