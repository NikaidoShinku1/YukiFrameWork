...
namespace YukiFrameWork
{
	...
	{
		private Dictionary<string, PropertyMethodDrawedInfo> methodInfoDicts = new Dictionary<string, PropertyMethodDrawedInfo>();
        ...
        private void InitMethodInfos(SerializedObject serializedObject, Type type)
        {
            ...
            foreach (MethodInfo methodInfo in methodInfos) 
            {
                ...
                ,out _,out _);    
                methodInfoDicts[methodInfo.Name] = new PropertyMethodDrawedInfo(serializedObject    ,target,methodInfo, method,color,enableEnumValueIfAttribute,disable,enableIf,disableIf,helperBox,runtimeDisabledGroup,editorDisabledGroup); 
            }         
        }     
        public override void OnInspectorGUI()
        {
            foreach (var methodName in methodInfoDicts.Keys)
            {
                PropertyMethodDrawedInfo info = methodInfoDicts[methodName];             
