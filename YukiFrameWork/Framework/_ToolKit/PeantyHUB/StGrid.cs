...
namespace YukiFrameWork
{
    ...
    public partial class StGrid
    {
        ...
        public double Hypotenuse => Math.Sqrt(cw * cw + ch * ch);
#else
        ...
        public void LinearIndexToTopRowMajor(int index,out int rTop,out int c)
        {
            int rBottom = index / colm;
             c = index % colm;
             rTop = row - 1 - rBottom;
        }
        /// <summary>
...
#if UNITY_2020_1_OR_NEWER
            float scale = sign * (float)Math.Sqrt(dragMagnitudeSquared / (w * w + h * h)) + 1;
#else
        ...
        /// <summary>
        /// 将网格的行列索引转换为线性索引（列主序 行索引从底部开始计数）
        /// </summary>
        /// <param name="rIndex">行索引</param>
        /// <param name="cIndex">列索引</param>
        /// <returns>线性索引</returns>
        ...
        public void CellIndexToCoordCenterByUp(int rIndex, int cIndex, out float x, out float y)
        {
            x = xMin + (cIndex + 0.5f) * cw;
            y = yMin - (rIndex + 0.5f) * ch;
        }
