...
可通过左上角YukiFrameWork/LocalWindow/事件分析器可视化所有的事件信息注册与销毁(安全性)
...
框架拥有自定义的可视化事件注册脚本组件:TypeEventListener、StringEventListener、EnumEventListener，继承后即可在组件进行自定义注册 示例如下:

``` csharp

using YukiFrameWork.Events;

public class TestEventListener : TypeEventListener<MyCustomArg>
{
